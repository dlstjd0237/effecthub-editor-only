...
function dumpToInputTag(p_slot){
		...
		xml=baseXML2Plist(p_slot,true,'cocos',xml_template);
		xmls[p_slot]=xml;
...
function keyShortCut(keycode){
  ...
  //p:80 plistDL   
  if (keycode == 80){ saveAs(dlName()+'.plist', xml, 'text/xml'); }
...
function downloadPng(p_slot){
	...
	if (!png_gz_b64[p_slot]) {
		...
		//gzip+base64 を戻してPNGとして保存 (was: PHPでgzdecodeしてもらっていた)
		var s=cc.Codec.GZip.gunzip(base64decode(png_gz_b64[p_slot]));
		var b=new Uint8Array(s.length);
		for (var i=0;i<s.length;i++) b[i]=s.charCodeAt(i) & 0xff;
		saveAs(dlName()+'.png', b, 'image/png');
	}
}

function downloadPlistNoImg(p_slot){

	saveAs(dlName()+'.plist', baseXML2Plist(p_slot,false,'cocos',xml_template), 'text/xml');
}

function downloadJsonNoImg(p_slot){

	saveAs(dlName()+'.json', baseXML2Plist(p_slot,false,'corona',corona_base_json), 'text/json');
}
